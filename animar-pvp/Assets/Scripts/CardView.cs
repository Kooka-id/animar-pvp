...
public class CardView : MonoBehaviour
{
    ...
    public static readonly Vector3 TARGET_POS_PLAYER = new Vector3(-130f, -600f, 0f); // Kiri Tengah
    public static readonly Vector3 TARGET_POS_OPPONENT = new Vector3(130f, -600f, 0f); // Kanan Tengah
    ...
    public static void ResetOverlayVisuals()
    {
        // Cari Overlay Panel (Harus sama dengan yang dicari di Start())
        GameObject overlayObj = GameObject.Find("OverlayPanel");
        ...
        if (overlayObj != null)
        {
            ...
            {
                // Reset Alpha ke 0
                LeanTween.alphaCanvas(overlayCG, 0f, 0.1f);
                
                // Matikan Interaksi dan Blocks Raycasts (Wajib)
                overlayCG.interactable = false;
                overlayCG.blocksRaycasts = false;
                
                // Posisikan Overlay ke belakang UI
                overlayCG.transform.SetAsFirstSibling();
                
                Debug.Log("[CardView] Global Overlay Visuals has been reset.");
            }
        }
        // Tidak perlu Debug.LogError jika overlayObj == null, karena sudah ada di Start()
    }
    ...
    public void ZoomCardToCenter(float zoomDuration = 0.5f)
    {
        // Cek dulu, kalau kartu belum ke-flip, gak usah di-zoom.
        if (!isFlipped)
        {
            Debug.LogWarning("Kartu belum terbuka (isFlipped = false). Gagal zoom.");
            return;
        }
        ...
        cardButton.interactable = false; // Nonaktifkan Button selama transisi
        ...
        LeanTween.scale(gameObject, targetScale, zoomDuration)
            ...
            .setOnComplete(() =>
            {
                Debug.Log("Kartu sudah ter-zoom dan terpusat.");
                transform.SetAsLastSibling(); // Pastikan di depan UI lain
                cardButton.interactable = true;
                ...
                Debug.Log("Kartu Anda di-zoom, siap berinteraksi atau menunggu lawan.");
            });
    }
